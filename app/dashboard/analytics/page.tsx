...
export default function AnalyticsPage() {
  ...
  const [tab, setTab] = useState<'expense' | 'income' | 'trend'>('expense')
  ...
  const fetchAnalytics = async () => {
    ...
    if (data) {
      ...
      const sourceMap = tab === 'expense' ? expTotals : incTotals
                  ...
                  {categoryTotals.map(cat => (
                    <div key={cat.name}>
                      ...
                      <div className="rounded-full h-1.5" style={{ backgroundColor: 'var(--bg3)' }}>
                        <div className="rounded-full h-1.5" style={{ width: `${grandTotal > 0 ? (cat.total / grandTotal) * 100 : 0}%`, background: cat.color }} />
                      </div>
                      <p className="text-xs mt-0.5 text-right" style={{ color: 'var(--text3)' }}>
